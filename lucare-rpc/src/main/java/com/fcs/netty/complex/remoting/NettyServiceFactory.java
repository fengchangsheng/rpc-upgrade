...
public class NettyServiceFactory {
    ...
    private static ConcurrentHashMap<String, NettyServiceLocator<?>> serviceCache = new ConcurrentHashMap<>();
...
//                    String key = business + "-" + service;
     ...
     * 如果缓存没有则创建
     */
    public static <T> NettyServiceLocator<T> foundLocator(Class<T> api) throws IOException, InterruptedException {
        String key = "locator-config-" + api.getName();
        String serviceName = api.getSimpleName();
        NettyServiceLocator<T> RPCServiceLocator = (NettyServiceLocator<T>) serviceCache.get(key);
        if (RPCServiceLocator == null) {
            synchronized (api) {
                RPCServiceLocator = (NettyServiceLocator<T>) serviceCache.get(key);
                if (RPCServiceLocator == null) {
                    ...
                    RPCServiceLocator = new NettyServiceLocator<T>(api, serviceName, host, port, context);
