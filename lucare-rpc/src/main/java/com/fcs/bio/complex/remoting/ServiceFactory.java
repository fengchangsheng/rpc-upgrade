...
public class ServiceFactory {
    ...
    private static ConcurrentHashMap<String, ServiceLocator<?>> serviceCache = new ConcurrentHashMap<>();
...
//                    String key = business + "-" + service;
    ...
    public static <T> ServiceLocator<T> foundLocator(Class<T> api) throws IOException, InterruptedException {
        String key = "locator-config-" + api.getName();
        String serviceName = api.getSimpleName();
        ServiceLocator<T> serviceLocator = (ServiceLocator<T>) serviceCache.get(key);
        if (serviceLocator == null) {
            synchronized (api) {
                serviceLocator = (ServiceLocator<T>) serviceCache.get(key);
                if (serviceLocator == null) {
                    ...
                    serviceLocator = new ServiceLocator<T>(api, serviceName, host, port, context);
                    serviceCache.put(key, serviceLocator);
                }
            }
        }
